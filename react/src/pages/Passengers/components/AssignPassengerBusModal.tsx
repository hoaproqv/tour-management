...
type AssignPassengerBusModalProps = {
  trip: Trip | null;
  open: boolean;
  onClose: () => void;
};

export default function AssignPassengerBusModal({
  trip,
  open,
  onClose,
  ...
  const { data: tripBusesResponse } = useQuery({
    queryKey: ["trip-buses", { trip: trip?.id }],
    queryFn: () => getTripBuses({ trip: trip?.id, page: 1, limit: 1000 }),
  ...
  useEffect(() => {
    if (!busesForTrip.length) {
      setSelectedTripBusId(undefined);
      return;
    }
    setSelectedTripBusId(busesForTrip[0].id);
  }, [busesForTrip]);

  const tripBusLabelMap = useMemo(() => {
    const map = new Map<string, string>();
    busesForTrip.forEach((tb) => {
      const label = tb.registration_number || tb.bus_code || `Bus #${tb.id}`;
      map.set(tb.id, label);
    });
    return map;
  }, [busesForTrip]);
  ...
  return (
    <Modal
      ...
      title={
        trip
          ? `Phân công xe cho hành khách - ${trip.name}`
          : "Phân công xe cho hành khách"
      }
      destroyOnClose
    >

...
export const NotificationDropdown: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "WARNING":
        return (
          <ExclamationCircleFilled className="text-amber-500 text-[22px]" />
        );
      case "SUCCESS":
        return <CheckCircleFilled className="text-emerald-500 text-[22px]" />;
      case "ERROR":
        return <ExclamationCircleFilled className="text-red-500 text-[22px]" />;
      default:
        return <InfoCircleFilled className="text-blue-500 text-[22px]" />;
    }
  };
                ...
                <div className="flex gap-4 items-start w-full relative">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      <Text
                        className={`text-[14px] font-semibold truncate pr-3 ${!item.is_read ? "text-slate-800" : "text-slate-600"}`}
                      >
                        {item.title}
                      </Text>
                      {!item.is_read && (
                        <div className="w-2.5 h-2.5 rounded-full bg-blue-500 flex-shrink-0 mt-1 shadow-[0_0_4px_rgba(59,130,246,0.5)]" />
                      )}
                    </div>
